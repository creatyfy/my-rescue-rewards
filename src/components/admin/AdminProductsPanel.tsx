...
import { AdminProduct, createProduct, deleteProduct, fetchAdminProducts, updateProduct } from "@/integrations/supabase/admin";
import { Check, Pencil, Plus, Trash2 } from "lucide-react";
import { calculateProductPoints, calculateProductValue, formatCurrency, formatPoints, PRODUCT_POINTS_MULTIPLIER } from "@/lib/points-config";
...
export function AdminProductsPanel() {
  ...
  const handleOpenEdit = (item: AdminProduct) => {
    ...
    setFormState({
      ...
      // Produto existente já está confirmado
  ...
  const handlePrizeValueChange = (value: string) => {
    ...
    const calculatedPoints = !isNaN(numericValue) && numericValue > 0 ? calculateProductPoints(numericValue) : 0;
    setFormState(prev => ({
      ...prev,
  ...
  const showConfirmationBox = formState.prizeValueReais && !isNaN(parseFloat(formState.prizeValueReais)) && parseFloat(formState.prizeValueReais) > 0 && !formState.pointsConfirmed;
  const calculatedPointsDisplay = formState.pointsCost ? formatPoints(Number(formState.pointsCost)) : "0";
  return <Card className="p-6">
      ...
      {loading ? <p className="text-sm text-muted-foreground">Carregando produtos...</p> : items.length === 0 ? <div className="rounded-lg border border-dashed border-muted-foreground/40 p-6 text-center text-sm text-muted-foreground">
              ...
              {items.map(item => <TableRow key={item.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      {item.image_url ? <img src={item.image_url} alt={item.name} className="h-10 w-10 sm:h-12 sm:w-12 rounded-lg border object-cover flex-shrink-0" /> : <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-lg border bg-muted flex-shrink-0" />}
                  ...
                  <TableCell className="whitespace-nowrap">
                    {item.prize_value_reais != null ? formatCurrency(item.prize_value_reais) : formatCurrency(calculateProductValue(item.points_cost))}
                  </TableCell>
                  <TableCell className="whitespace-nowrap">
                    <div className="flex flex-col">
                      <span>{formatPoints(item.points_cost)} pts</span>
                      {item.points_manual_edit && <span className="text-xs text-muted-foreground">(editado)</span>}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      <Button variant="outline" size="sm" className="border-destructive/50 text-destructive hover:bg-destructive/10" onClick={() => handleDelete(item.id)}>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4">
            <div className="rounded-xl border border-border/50 p-4">
              ...
              <div className="flex items-center gap-4">
                <div className="h-20 w-20 rounded-xl border overflow-hidden bg-muted">
                  {localPreviewUrl || formState.imageUrl ? <img src={localPreviewUrl ?? formState.imageUrl} alt={formState.name || "Preview do produto"} className="h-full w-full object-cover" loading="lazy" /> : null}
                </div>
            ...
            <div className="grid gap-2">
              ...
              <Input id="product-name" value={formState.name} onChange={event => setFormState(prev => ({
              ...prev,
            ...
            <div className="grid gap-2">
              ...
              <Textarea id="product-description" value={formState.description} onChange={event => setFormState(prev => ({
              ...prev,
            ...
            <div className="grid gap-2">
              ...
              <Input id="product-prize-value" type="number" min={0} step="0.01" placeholder="Ex: 500.00" value={formState.prizeValueReais} onChange={event => handlePrizeValueChange(event.target.value)} />
            ...
            <div className="grid gap-2">
              ...
              <Input id="product-points" type="number" min={0} value={formState.pointsCost} onChange={event => handlePointsCostChange(event.target.value)} disabled={!formState.pointsManualEdit && !formState.pointsConfirmed} className={!formState.pointsManualEdit && !formState.pointsConfirmed ? "bg-muted" : ""} />
            </div>

            {/* Bloco de confirmação */}
            {showConfirmationBox && !formState.pointsManualEdit && <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
            ...
            {formState.pointsManualEdit && !formState.pointsConfirmed && <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4">
                <p className="text-sm text-foreground mb-3">
            ...
            {formState.pointsConfirmed && <div className="rounded-lg border border-green-500/30 bg-green-500/5 p-3 flex items-center gap-2">
            ...
            <div className="grid gap-2">
              ...
              <Input id="product-stock" type="number" min={0} value={formState.stock} onChange={event => setFormState(prev => ({
              ...prev,
            ...
            <div className="grid gap-2">
              ...
              <Input id="product-image" type="file" accept="image/*" onChange={event => setFormState(prev => ({
              ...prev,
              imageFile: event.target.files?.[0] ?? null
            }))} />
              <Label htmlFor="product-image-url">Imagem (URL)</Label>
              <Input id="product-image-url" value={formState.imageUrl} onChange={event => setFormState(prev => ({
              ...prev,
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              ...
              <Switch checked={formState.active} onCheckedChange={checked => setFormState(prev => ({
              ...prev,
              active: checked
            }))} />
            </div>
