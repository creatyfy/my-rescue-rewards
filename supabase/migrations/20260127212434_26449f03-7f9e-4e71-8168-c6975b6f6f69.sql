-- Update submit_receipt function to award 10 points per R$1 spent
CREATE OR REPLACE FUNCTION public.submit_receipt(p_qr_code_token text, p_purchase_value numeric, p_image_path text)
 RETURNS TABLE(receipt_id uuid, protocol_number text, points_earned integer, status receipt_status)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- NEW RULE: 10 points per R$1 spent
  v_points := floor(p_purchase_value * 10)::int;
